...
from qichacha.conf import QICHACHA_COOKIE
from qichacha.conf import KEY_WORDS_PATH, SEARCH_URL
import re
import time
import random
import sys
...
class QichachaSpider(BaseSpider):
    ...
    def start_requests(self):
        ...
        while len(self.key_set) != 0:
            ...
            yield Request(url, cookies=QICHACHA_COOKIE[0],
    ...
    def get_content_request(self, response):
        ...
        if response.body.find(s) != -1 or response.body.find("您的操作过于频繁，验证后再操作") != -1:
            # self.key_set.add(key)
            kop = open("./data/search_keyword1.log", "a")
            kop.write(response.body)
            kop.close()
        ...
        if response.body.find("您的搜索词太宽泛，建议更换一下搜索词") != -1 or (
            ...
            erroroutput = open("./data/error1.txt", "a")
    ...
    def parse_url(self, response):
        ...
        if response.body.find("您的搜索词太宽泛，建议更换一下搜索词") != -1 or (
            ...
            source.close()
            rop = open("./data/key_words_search1.txt", "a")
            ...
            kop = open("./data/logs1.txt", "a")
            kop.write(response.request.url + "\t" + response.body + "\t" + key.replace("\n", "") + "\n")
            kop.close()
